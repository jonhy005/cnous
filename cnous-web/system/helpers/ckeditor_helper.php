...
function cke_create_instance($data = array()) {
	...
	if (isset ( $data ['config'] )) {
		foreach ( $data ['config'] as $k => $v ) {
			...
			$arrayKeysValues = array_keys ( $data ['config'] );
			if ($k !== end ( $arrayKeysValues )) {
				$return .= ",";
			}
		}
	}
		...
		foreach ( $data ['styles'] as $k => $v ) {
			...
			if (isset ( $v ['styles'] )) {
				foreach ( $v ['styles'] as $k2 => $v2 ) {
					...
					$arrayKeysStylesValues = array_keys ( $v ['styles'] );
					if ($k2 !== end ( $arrayKeysStylesValues )) {
						$return .= ",";
					}
				}
			}
			...
			$arrayKeysValues = array_keys ( $data ['styles'] );
			if ($k !== end ( $arrayKeysValues )) {
				$return .= ',';
			}
		}
	...
	foreach ( $data as $key ) {
		if (is_array ( $key )) {
			...
			foreach ( $key as $string ) {
				$return .= "'" . $string . "'";
				$arrayKeyValues = array_values ( $key );
				if ($string != end ( $arrayKeyValues )){
					$return .= ",";
				}
		...
		$arrayDataValues = array_values ( $data );
		if ($key != end ( $arrayDataValues )){
			$return .= ",";
		}
